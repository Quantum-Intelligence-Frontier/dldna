...
from expertai_src.chapter_07.transformer.config import TransformerConfig
from expertai_src.chapter_07.transformer.transformer import Transformer
...
def create_copy_data(config, batch_size):
    ...
    input_seq = torch.randint(1, config.vocab_size, (batch_size, seq_length))
    return input_seq
...
def train_copy_task(
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(steps_per_epoch):
            # 데이터 생성 및 GPU로 이동
            input_seq = create_copy_data(config,batch_size).to(device)
